...
if( !empty($_POST['email']) && !empty($_POST['id'])  ){
    ...
    if($con){
        ...
        $sql_query_select="SELECT * from friend_tabel where (id_user1=".$id." or id_user2=".$id." ) and type_friend=0;";
        ...
        //$sql_query_select="SELECT * FROM users_tabel";
        $res=mysqli_query($con,$sql_query_select);
        $i=0;

        if(mysqli_num_rows($res)!=0 ){
            while($row=mysqli_fetch_assoc($res)){
                $result["array"][$i]=array("email"=>$row['email'],"name"=>$row['name'],"birthday"=>$row['birthday']);
                $i++;
            }
            $result["result"]="success";
        }else{
