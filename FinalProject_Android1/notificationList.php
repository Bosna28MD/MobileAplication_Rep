...
if( !empty($_POST['email']) && !empty($_POST['id'])  ){
    ...
    if($con){
        ...
        $sql_query_select="SELECT * from friend_tabel where (id_user1=".$id." or id_user2=".$id." ) and type_friend=-1 and user_send!=".$id." ;";
        ...
        /*else{
            $result["result"]="No notification";
        }*/
        $result["result"]="success";
        

    }else{
        $result["result"]="fail connection to database";
    }

}else{
    $result["result"]="fail empty data inserted";
}
